...
class MPC:
    ...
    def cost_function(
        ...
        for i in range(self.horizon):
            interp = i / self.horizon
            stage_ref = ref.copy()
            stage_ref[3] = ref[3] * interp

            error = traj[i] - stage_ref
    ...
    def solve_control(
        ...
        predicted_traj = [
            self._to_pos(s) for s in self.predict_trajectory(start_arr, result.x)
        ]
